...
class DatasetCreator(Dataset):
    ...
    def __init__(self, pics_root_dir, labels_root_dir, obj_percentage=20,
                 amp_data=True, complex_data=False, transform=None):
    ...
    def __getitem__(self, item):
        ...
        if eps < self.obj_percentage:
            item = int(random.choice(np.arange(list(labels).index(0) - 1)))

        else:
            item = int(random.choice(np.arange(list(labels).index(0), len(labels))))
...
if __name__ == '__main__':
    db = DatasetCreator(pics_root_dir="../../signal_labels_new/patches_5000_grand/learning/",
                        ...
                        obj_percentage=50,
                        amp_data=True, complex_data=False)  # load database
